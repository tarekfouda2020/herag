...
class UserProductsData{
 ...
 void setAddUserToFollowers(BuildContext context,String userId)async{
  ...
  void setUserRate(BuildContext context,String userId,AddOwnerCubit addOwnerCubit){
    var auth= context.read<AuthCubit>().state.authorized;
    if(auth){
  ...
  void navigateToChat(BuildContext context,String userId,String userName,UserModel user){
    var auth= context.read<AuthCubit>().state.authorized;
    if(auth){
      AutoRouter.of(context).push(ChatRoute(senderId: user.id,receiverId: userId,userName: userName)
      );
    }else{
      LoadingDialog.showAuthDialog(context: context);
    }
