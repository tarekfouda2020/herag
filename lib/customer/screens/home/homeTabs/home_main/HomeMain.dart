...
class _HomeMainState extends State<HomeMain>  with SingleTickerProviderStateMixin,HomeMainData {

  @override
  void initState() {
    super.initState();
    HomeMainData.tabController= new TabController(length: widget.parentCount, vsync: this);
  }
                  ...
                  children: [
                    Container(
                      decoration: BoxDecoration(
                        ...
                        border: Border(
                          bottom: BorderSide(color: MyColors.greyWhite.withOpacity(1))
                        )
                      ),
                      child: TabBar(
                        ...
                        tabs: List.generate(snapshot.data.length, (index) {
                          return Tab(child: MyText(title: snapshot.data[index].name,size: 12,),);
                        })
                      ),
                    ),
                    Flexible(
  ...
  Widget _buildAppbar(){
    return AppBar(
      ...
      toolbarHeight: 70,
      leading: IconButton(
        icon: Icon(Icons.menu,size: 30,color: MyColors.white,),
        onPressed: ()=> widget.scaffold.currentState.openDrawer(),
      ),
      actions: [
        IconButton(
          icon: Icon(Icons.view_quilt_sharp,size: 30,color: MyColors.white,),
                ...
                Visibility(
                  ...
                  child: BlocBuilder<CatTabCubit,CatTabState>(
                    ...
                    builder: (context,state){
                      return _buildCategoryView(snapshot.data,parent,state.id);
                    },
                  ),
                ),
                  ...
                  builder: (context,state){
                    return Column(
                      children: List.generate(state.catRows.length, (index) {
                        return _buildSubCategoryView(state.catRows[index].cats,state.catRows[index].parentId);
                      }),
                    );
                  },
                ),
                Flexible(child: HomeMainAds(catId: parent,)),
              ],
            ),
          );
  ...
  Widget _buildCatItem(Category model,int index,int current){
    ...
    Color textColor = model.id==current?MyColors.white:MyColors.blackOpacity;
    return InkWell(
      ...
      child: Container(
        ...
        child:  MyText(title: "${model.name}",size: 10,color: textColor,),
      ),
    );
  }
