...
class EndDrawer extends StatelessWidget {
  ...
  void setSelectCat(Category model) async {
    ...
    List<Category> children = await scaffold.currentContext
        .read<MyDatabase>()
        .selectChildrenCatsAsync(model.id);
    if (children.length > 0) {
      HomeMainData.catViewCubit
          .onAddCatRow(CategoryChildModel(cats: children, parentId: model.id));
    }
                      ...
                      return Column(
                        children: [
                          ...
                          StreamBuilder<List<Category>>(
                            stream: context
                                .watch<MyDatabase>()
                                .selectSlideChildrenCats(snapshot.data[index]),
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
                                return Container(
